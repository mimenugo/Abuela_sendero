const WHATSAPP_NUMBER = "526642352838";

let products = [];
let cart = JSON.parse(localStorage.getItem("chana-cart") || "[]");
...
async function loadProducts() {
  ...
  try {
    ...
    if (Array.isArray(window.CHANA_CATALOG_BACKUP) && window.CHANA_CATALOG_BACKUP.length) {
      products = structuredClone(window.CHANA_CATALOG_BACKUP)
        .filter(product => product.active !== false)
        .sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
      cart = cart.filter(item => products.some(product => product.id === item.id));
      saveCart();
...
function sendOrder(event) {
  ...
  const message = [
    "*NUEVO PEDIDO - COCINA DE LA ABUELA CHANA*",
    "",
    productLines,
    "",
